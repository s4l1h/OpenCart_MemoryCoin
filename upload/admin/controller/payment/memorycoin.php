...
class ControllerPaymentMemoryCoin extends Controller {
	...
	private function validate() {
        ...
        if (!$this->request->post['memorycoin_rpc_username']) {
            $this->error['username'] = $this->language->get('error_username');
        }
        if (!$this->request->post['memorycoin_rpc_address']) {
            $this->error['address'] = $this->language->get('error_address');
        }
        if (!$this->request->post['memorycoin_rpc_password']) {
            $this->error['password'] = $this->language->get('error_password');
        }
        if (!$this->request->post['memorycoin_rpc_port']) {
            $this->error['port'] = $this->language->get('error_port');
        }
        if (!$this->request->post['memorycoin_prefix']) {
            $this->error['prefix'] = $this->language->get('error_prefix');
        }
        if (!$this->request->post['memorycoin_countdown_timer']) {
	...
	public function index() {
        ...
        $this->data['entry_show_mmc']         = $this->language->get('entry_show_mmc');
        $this->data['entry_mmc_decimal']         = $this->language->get('entry_mmc_decimal');
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('payment/memorycoin', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => ' :: '
        );
		...
		$rows = $result->rows;
		$max = $result->num_rows;
		$memorycoin_total_in_db = 0;
		$memorycoin_address_in_db = 0;
		for($i = 0;$i < $max;$i++) {
			if($rows[$i]["Field"] == "memorycoin_total") {
				$memorycoin_total_in_db = 1;
			}
			if($rows[$i]["Field"] == "memorycoin_address") {
				$memorycoin_address_in_db = 1;
			}
		}
		if(!$memorycoin_total_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD memorycoin_total DOUBLE AFTER currency_value;");
		}
		if(!$memorycoin_address_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD memorycoin_address VARCHAR(34) AFTER memorycoin_total;");
		}
        ...
        if (isset($this->request->post[$this->payment_module_name.'_show_mmc'])) {
			$this->data[$this->payment_module_name.'_show_mmc'] = $this->request->post[$this->payment_module_name.'_show_mmc'];
		} else {
			$this->data[$this->payment_module_name.'_show_mmc'] = $this->config->get($this->payment_module_name.'_show_mmc');
		}

        if (isset($this->request->post[$this->payment_module_name.'_mmc_decimal'])) {
			$this->data[$this->payment_module_name.'_mmc_decimal'] = $this->request->post[$this->payment_module_name.'_mmc_decimal'];
		} else {
			$this->data[$this->payment_module_name.'_mmc_decimal'] = $this->config->get($this->payment_module_name.'_mmc_decimal');
		}
